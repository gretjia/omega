...
class OmegaTrainerV3:
    ...
    def _prepare_frames(self, df: pl.DataFrame, cfg: L2PipelineConfig) -> pl.DataFrame:
        ...
        # Keep binary semantics when configured: remove neutral labels before model fit.
        if bool(getattr(tcfg, "drop_neutral_labels", False)):
...
def evaluate_frames(df: pl.DataFrame, cfg: L2PipelineConfig) -> Dict[str, float]:
    ...
    # 3. Vector Alignment (direction consistency)
    # Historical audit semantics: sign(direction) vs sign(future return) hit-rate.
    # Use kernel direction when available; fallback to sign(topo_area).
    dir_expr: pl.Expr | None = None
    if "direction" in df.columns:
        dir_expr = pl.col("direction")
    elif "topo_area" in df.columns:
        dir_expr = pl.col("topo_area").sign()

    if dir_expr is not None and "close" in df.columns:
        horizon = max(1, int(getattr(cfg.validation, "forward_return_horizon_buckets", 1)))
        min_samples = max(1, int(getattr(cfg.validation, "min_samples", 30)))
        aligned = (
            df.with_columns(
                [
                    dir_expr.alias("_dir_sign"),
                    (pl.col("close").shift(-horizon) - pl.col("close")).alias("_fwd_ret"),
                ]
            )
            .select(["_dir_sign", "_fwd_ret"])
            .drop_nulls()
        )
        if aligned.height >= min_samples:
            dir_sign = np.sign(np.nan_to_num(aligned["_dir_sign"].to_numpy(), nan=0.0))
            fwd_sign = np.sign(np.nan_to_num(aligned["_fwd_ret"].to_numpy(), nan=0.0))
            mask = (dir_sign != 0.0) & (fwd_sign != 0.0)
            if int(np.sum(mask)) >= min_samples:
                align = float(np.mean(dir_sign[mask] == fwd_sign[mask]))
            else:
                align = 0.0
        else:
            align = 0.0
