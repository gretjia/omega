...
## Windows 1 Execution & Environment Hazards (Addendum)
- **Extreme V63 Windows Bottleneck (Fallback Path)**: The `stage2_physics_compute.py` script contains a hardcoded fallback (`force_scan_fallback = sys.platform == "win32"`) intended to prevent memory crashes on older versions. However, this fallback processes files by spawning thousands of individual Python subprocesses per day file (one per symbol). On V63, this scales exponentially and will NEVER finish (taking 15+ hours per file before hitting the timeout wall).
- **The Fix**: You MUST set `$env:OMEGA_STAGE2_FORCE_SCAN_FALLBACK='0'`, `$env:OMEGA_DISABLE_NUMBA='0'`, and `$env:OMEGA_STAGE2_ISOLATE_SYMBOL_BATCH='0'` in PowerShell before launching the Windows Stage 2 pipeline. This unlocks raw Numba C-level computation speed on Windows, restoring parity with the Linux node's processing times.

## Supervisor Daemon Environment Bug (GBK Decoding)
- **Windows GBK Encoding Crash**: `stage2_targeted_supervisor.py` uses `subprocess.run(..., capture_output=True, text=True)`. On Windows, the default console encoding is often `GBK` (cp936). When the child process outputs binary strings or special UTF-8 characters via `Numba/Polars`, the supervisor instantly crashes with `UnicodeDecodeError: 'gbk' codec can't decode byte...`, breaking the infinite restart loop and killing the entire node.
- **The Fix**: The supervisor script has been permanently patched to enforce `encoding="utf-8"` and `errors="replace"` on all `subprocess.run` calls to guarantee cross-platform string safety.
